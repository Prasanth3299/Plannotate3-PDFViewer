...
")]
	...
	protected DialogHandler dialogHandler;
		...
		onConsoleMessage += (message, source) => {
			...
			//Debug.Log(text, this);
			StartCoroutine(LoadingScreenDisable());
			if (previousUrl != Url)
			{
				previousUrl = Url;
				if (Url.Contains("annotaion_clicked=true"))
				{
					print("yes");
					string[] str = Url.Split('&');
					string annotation = str[str.Length - 1];
					print(annotation);
					annotation = annotation.Remove(annotation.Length - 1,1);
					string layer = str[str.Length - 2];

					string[] str1 = annotation.Split('=');
					string[] str2 = layer.Split('=');
					annotationClicked = true;
					annotationId = str1[str1.Length - 1];
					layerId = str2[str2.Length - 1];
					print(annotationId);
					print(layerId);
			...
			//loadingScreen.SetActive(false); // prasanth added
		};
	...
	public bool IsReady {
		get { return browserId != 0; }
		 ...
		 IEnumerator LoadingScreenDisable() // prasanth added
		{
...
#pragma warning restore 618
	...
	internal bool DeferUnready(Action ifNotReady) {
		if (browserId == 0) {
			...
			print("ready"); // prasanth added
			StartCoroutine(LoadingScreenDisable()); // prasanth added
			CheckSanity();
			return false;
		}
	}
